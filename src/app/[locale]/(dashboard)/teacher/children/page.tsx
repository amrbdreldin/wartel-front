...
import { ParentHeader } from "@/app/[locale]/(dashboard)/parent/_components/ParentHeader";
...
export default function TeacherChildrenPage() {
  ...
  const handleAddStudent = async (
    ...
    try {
      ...
      toast.success(resObj?.message || t("childAddedSuccess"));
  ...
  const handleViewProfile = (childId: number) => {
    router.push(`/${locale}/teacher/children/${childId}/profile-details`);
  };
  ...
  const handleLoginAsChild = (child: { id: number; name: string }) => {
    ...
    loginAsStudent(child.id, {
      onSuccess: (res) => {
        ...
        if (token) {
          switchToChild({
            user: res.user || { id: child.id, full_name: child.name, role: UserRole.STUDENT, phone: "" },
            accessToken: token,
