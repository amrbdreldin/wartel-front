  ...
  return (
    <div className="bg-card rounded-3xl p-6 border border-border/50 shadow-[0_8px_30px_rgb(0,0,0,0.02)] hover:shadow-[0_20px_40px_rgb(0,0,0,0.05)] dark:hover:shadow-[0_20px_40px_rgb(0,0,0,0.25)] hover:-translate-y-1.5 transition-all duration-500 group flex flex-col relative overflow-hidden h-full">
      ...
      <div className="flex items-center gap-2 mb-4">
        <h3 className="font-extrabold text-lg text-foreground tracking-tight">{t("student.warningsTitle") || t("nav.warnings")}</h3>
        {warningCount > 0 && (
      ...
      {/* Overlay Link — routes to the notifications page */}
      <Link href={`/${locale}/student/notifications`} className="absolute inset-0 z-10" aria-label={t("student.goToNotifications") || "عرض الإشعارات"}></Link>
    </div>
  );
}
