...
export function StudentHero({
  ...
  // Determine active stats values
  const trackName = nextSession?.group?.name || nextSession?.group_name || t("noSession");
  const buddyName = tamamCard?.buddy?.full_name || t("noBuddy");
  const gradePercentage = lastGrade?.score
    ? parseFloat(lastGrade.score).toFixed(0) + "%"
    : t("noGrade");
        ...
        <div className="flex flex-col lg:flex-row items-center lg:items-start justify-between gap-6 lg:gap-8">
          ...
          <div className="flex flex-col items-center lg:items-start text-center lg:text-start gap-2 sm:gap-4 flex-1">
            ...
            <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl font-black text-white tracking-tight leading-tight">
              {t("welcomeTitle", { name: studentName })}
            </h1>
          ...
          <div className="grid grid-cols-3 lg:flex gap-2 sm:gap-4 w-full lg:w-auto justify-center">
            {[
              {
                label: t("currentTrack"),
                value: trackName,
                icon: BookOpen,
                color: "from-emerald-500/30 to-emerald-400/10",
              },
              {
                label: t("recitationBuddy"),
                value: buddyName,
                icon: Users,
              ...
              >
                ...
                <p className="text-sm sm:text-base md:text-lg font-black text-white truncate max-w-full" title={stat.value}>
                  {stat.value}
                </p>
